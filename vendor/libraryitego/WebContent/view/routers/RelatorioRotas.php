 ...
 * @relatorioComprovanteMulta=btc
 */
class RelatorioRotas 
{

	
	public function relatorioComprovanteMulta($idmulta)
	{
		...
		$selectmulta = $relatorio->select(new Multa(),true,array(),array(),array('idmulta' => $idmulta));
		...
		if ($quantidade[0]['COUNT(*)'] == 2) {
			...
			$selectAluno = $relatorio->select(new Aluno(),true,array('usuario' => array()),array(),array('usuario_idusuario' => $selectEmprestimo[0]['idusuario']));

			$selectEmprestimo2 =  $relatorio->select(new Emprestimo(),true,array('usuario' => array(),'patrimonio' => array('livro' => array())),array(),array('data_devolucao' => "'".$atual."'", 'idusuario' => $selectEmprestimo[0]['idusuario']));

			if (!empty($selectAluno)) {

			$selectTurmaHasAluno = $relatorio->select(new Turma_has_Aluno(),true,array('turma' => array('curso' => array(), 'turno' => array()),'aluno' => array()),array(),array('idaluno' => $selectAluno[0]['idaluno']),array('data_inicio' => "desc"));

			$previsao_devolucao = new \DateTime($selectEmprestimo2[0]['data_emprestimo'],new \DateTimeZone('America/Sao_Paulo'));

			$previsao_devolucao->add(new \DateInterval('PT168H'));

			$previsao =  $previsao_devolucao->format('Y-m-d');

			$data_devolucao = new \DateTime($selectEmprestimo2[0]['data_devolucao'],new \DateTimeZone('America/Sao_Paulo'));

			$devolucao =  $data_devolucao->format('Y-m-d');

			$resultado = $control->contaData($devolucao,$previsao);
			...
			if ($multa_resultado > 0 ) {
				$selectvalor = $relatorio->select(new Valor(),false,array(),array(),array(),array('idvalor' => "desc"));

				$valor = $multa_resultado * $selectvalor[0]['valor_diario_multa'] * $quantidade[0]['COUNT(*)'];    

				$cpf = $relatorio->mask($selectAluno[0]['cpf'],'###.###.###-##'); 


			$html = $rain->setConteudo(array("header","comprovante_pagamento_aluno","footer"),array(

			
			'nome_usuario' => $selectEmprestimo2[0]['nome_usuario'],
			'cpf' => $cpf,
			'matricula' => $selectTurmaHasAluno[0]['matricula'],
			'nome_livro' => $selectEmprestimo2[0]['nome_livro'].",",
			'nome_livro2' => $selectEmprestimo2[1]['nome_livro'],
			'nome_curso' => $selectTurmaHasAluno[0]['nome_curso'],
			'nome_turno' => $selectTurmaHasAluno[0]['nome_turno'],
			'data_devolucao' => Convert::date_to_string($selectEmprestimo2[0]['data_devolucao']),
			'valor' => $valor.",00"
		...
		$relatorio->callMpdf($html);
			...
			$selectFuncionario = $relatorio->select(new Funcionario(),true,array('usuario' => array(),'cargo' => array()),array(),array('usuario_idusuario' => $selectEmprestimo2[0]['idusuario']));
				...
				if ($multa_resultado > 0 ) {
					...
					$cpf = $relatorio->mask($selectFuncionario[0]['cpf'],'###.###.###-##'); 


				$html = $rain->setConteudo(array("header","comprovante_pagamento_funcionario","footer"),array(
		...
		$selectEmprestimo3 =  $relatorio->select(new Emprestimo(),true,array('usuario' => array(),'patrimonio' => array('livro' => array())),array(),array('data_devolucao' => "'".$atual."'", 'idusuario' => $selectEmprestimo[0]['idusuario']));
		...
		if (!empty($selectAluno)) {
			...
			$previsao_devolucao = new \DateTime($selectEmprestimo3[0]['data_emprestimo'],new \DateTimeZone('America/Sao_Paulo'));

			$previsao_devolucao->add(new \DateInterval('PT168H'));

			$previsao =  $previsao_devolucao->format('Y-m-d');

			$data_devolucao = new \DateTime($selectEmprestimo3[0]['data_devolucao'],new \DateTimeZone('America/Sao_Paulo'));
			...
			if ($multa_resultado > 0 ) {
				...
				$valor = $multa_resultado * $selectvalor[0]['valor_diario_multa'];
			...
			'nome_usuario' => $selectEmprestimo3[0]['nome_usuario'],
			'cpf' => $cpf,
			'matricula' => $selectTurmaHasAluno[0]['matricula'],
			'nome_livro' => $selectEmprestimo3[0]['nome_livro'],
			'nome_livro2' => "",
			'nome_curso' => $selectTurmaHasAluno[0]['nome_curso'],
			'nome_turno' => $selectTurmaHasAluno[0]['nome_turno'],
			'data_devolucao' => Convert::date_to_string($selectEmprestimo3[0]['data_devolucao']),
			'valor' => $valor.",00"
		...
		$relatorio->callMpdf($html);
			...
			echo "Erro no sistema!";
				...
				$selectFuncionario = $relatorio->select(new Funcionario(),true,array('usuario' => array(),'cargo' => array()),array(),array('usuario_idusuario' => $selectEmprestimo3[0]['idusuario']));
	...
	public function pdfDevolucaoEmprestimo($idusuario){
		...
		$multa = $relatorio->select(new multa(),false);
