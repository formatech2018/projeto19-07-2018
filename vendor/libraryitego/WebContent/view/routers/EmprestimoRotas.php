...
class EmprestimoRotas 
{
	...
	public function post_insert()
	{
		...
		if (!empty($selectUsuario)) {
			...
			if ($nemprestimos>0) {
					...
					if (!empty($res[1])) {
						EmprestimoRotas::sendEmailPdf($res[1]);
					}
					else{
						EmprestimoRotas::sendEmailPdf($res[2]);
					}
					$email = new Email();
	...
	public function sendEmailPdf($usuario){
	...
	public function disabled_emprestimo($idemprestimo){
		...
		if (!empty($res)) {
			...
			if ($multa_resultado > 0 ) {
				...
				$rain->setConteudo(array("verifica_multa","pagamento_multa"),array(
				...
				RelatorioRotas::pdfDevolucaoEmprestimo($selectEmprestimo[0]['idusuario']);
				...
				$rain->setConteudo(array("verifica_multa"),array(
	...
	public function disabled_all_emprestimo(){
		...
		if (count($res) == count($_POST)) {
			...
			if ($multa_resultado > 0 ) {
				...
				$rain->setConteudo(array("verifica_multa2","pagamento_multa"),array(
